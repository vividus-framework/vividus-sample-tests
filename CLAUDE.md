...
`configuration-set.active` is empty by default — running tests without setting it (or overriding it in `overriding.properties`) fails by design.
...
./gradlew runStories -Pvividus.configuration-set.active=electron-app # Electron-based application (VS Code is a system under test)
```
